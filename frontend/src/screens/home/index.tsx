// Keep this import section as-is
...
export default function Home() {
  ...
  const eraserCursorRef = useRef<HTMLDivElement>(null); // Ref for eraser preview
  ...
  const [pages, setPages] = useState<
    Array<{ id: string; name: string; thumbnail: string; content: string }>
  >([]);
  ...
  const saveCurrentPage = useCallback(() => {
    ...
    localStorage.setItem(
      "pages",
      JSON.stringify(
        pages.map((p) => (p.id === currentPage.id ? updatedPage : p))
      )
    );
  ...
  return (
    <>
      ...
      <div className="flex justify-around py-5 gap-2 ml-16">
        <Button onClick={() => setReset(true)} className="z-20 bg-black text-white hover:bg-slate-500">Reset</Button>
        <Group className="z-20">
          {SWATCHES.map((swatch) => (
            <ColorSwatch key={swatch} color={swatch} style={{ cursor: "pointer" }}
              onClick={() => {
                setColor(swatch);
                setIsEraser(false);
              }}
        ...
        <div className="z-20 flex items-center gap-2">
          <Button onClick={() => setIsEraser(!isEraser)} className={`bg-black text-white hover:bg-slate-500${isEraser ? " bg-gray-700" : ""}`} variant="default">
            <Eraser size={20} />
          </Button>
          {isEraser && (
            <Slider value={eraserSize} onChange={setEraserSize} min={5} max={50}
              label={(value) => `${value}px`} className="w-32" />
          )}
        </div>
        <Button onClick={runRoute} className="z-20 bg-black text-white hover:bg-slate-500">Run</Button>
        <Button className="text-white bg-red-500 z-20 hover:bg-red-900" onClick={handleLogout}>Logout</Button>
      </div>
      ...
      {isEraser && (
        <div
          ref={eraserCursorRef}
          className="pointer-events-none fixed z-30 rounded-full border-2 border-white bg-white bg-opacity-20"
          style={{
            width: `${eraserSize}px`,
